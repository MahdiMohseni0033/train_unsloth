...
# ── Kelvin2 scratch install root ──
# Keep the virtual environment, launchers, package caches, build temp files,
# and model caches out of the quota-limited login home directory.
UNSLOTH_INSTALL_ROOT="${UNSLOTH_INSTALL_ROOT:-/mnt/scratch2/users/mmohseni}"
mkdir -p "$UNSLOTH_INSTALL_ROOT"
UNSLOTH_INSTALL_ROOT=$(CDPATH= cd "$UNSLOTH_INSTALL_ROOT" && pwd)
...
STUDIO_HOME="$UNSLOTH_INSTALL_ROOT/.unsloth/studio"
...
create_studio_shortcuts() {
    ...
    _css_data_dir="$XDG_DATA_HOME/unsloth"
...
DATA_DIR="$XDG_DATA_HOME/unsloth"
...
LAUNCHER_EOF
    ...
    if [ "$_css_os" = "linux" ]; then
        # ── Linux: .desktop file ──
        _css_app_dir="$XDG_DATA_HOME/applications"
...
STUB_EOF
        ...
        _css_ps1_tmp=$(mktemp "$TMPDIR/unsloth-shortcut-XXXXXX.ps1" 2>/dev/null) || true
...
if ! command -v uv >/dev/null 2>&1 || ! _uv_version_ok uv; then
    ...
    rm -f "$_uv_tmp"
    if [ -f "$UNSLOTH_INSTALL_ROOT/.local/bin/env" ]; then
        . "$UNSLOTH_INSTALL_ROOT/.local/bin/env"
    fi
...
" >/dev/null 2>&1; then
        ...
        echo "   Moved legacy Studio environment to $VENV_DIR"
        _MIGRATED=true
    else
...
_CERTIFI_CA=$("$VENV_DIR/bin/python" -c "import certifi; print(certifi.where())" 2>/dev/null || echo "")
if [ -n "$_CERTIFI_CA" ] && [ -f "$_CERTIFI_CA" ]; then
...
# Keep the Python/ROCm training environment successful on Kelvin2 even when
# Studio's optional GGUF llama.cpp source fallback is unavailable.
_SKIP_BASE=0
_SKIP_GGUF_SOURCE_BUILD=true
if [ "${UNSLOTH_SKIP_GGUF_BUILD:-1}" = "0" ]; then
    _SKIP_GGUF_SOURCE_BUILD=false
fi
_SETUP_EXIT=0
...
if [ "$STUDIO_LOCAL_INSTALL" = true ]; then
    ...
    _SKIP_GGUF_BUILD="$_SKIP_GGUF_SOURCE_BUILD" \
...
# ── Make 'unsloth' available from the scratch-local bin directory ──
mkdir -p "$UNSLOTH_INSTALL_ROOT/.local/bin"
ln -sf "$VENV_DIR/bin/unsloth" "$UNSLOTH_INSTALL_ROOT/.local/bin/unsloth"

_LOCAL_BIN="$UNSLOTH_INSTALL_ROOT/.local/bin"
case ":$PATH:" in
    ...
    *)
        ...
        if [ -n "$_SHELL_PROFILE" ]; then
            if ! grep -q '\.local/bin' "$_SHELL_PROFILE" 2>/dev/null; then
                ...
                echo 'export PATH="${UNSLOTH_INSTALL_ROOT:-/mnt/scratch2/users/mmohseni}/.local/bin:$PATH"' >> "$_SHELL_PROFILE"
                step "path" "added scratch-local bin to PATH in $_SHELL_PROFILE"
            fi
        fi
